...
function CountryHolidaysPage({
  ...
  const countryQuery = useQuery<ICountry, Error>(["country", alpha2Code], () =>
    getCountryByAlpha2Code(alpha2Code)
  );
  const availableLanguageQuery = useQuery<ILanguage[], Error>(
    "availableLanguage",
    getAvailableHolidaysLanguages
  );
  const allHolidaysQuery = useQuery(
    ["allHolidays", alpha2Code, langFromForm],
    () =>
      getAllHolidays(
        alpha2Code,
        checkLanguageFallback(getLanguageFromApp(langFromForm), languages)
      )
  );
  ...
  function addHolidayToTile(tileDate: Date) {
    if (filteredHolidays && checkboxState) {
      return filteredHolidays.map(
        (holiday: IHoliday) =>
          holiday.date === formatDate(tileDate) && holiday.name
      );
    }
    return allHolidaysQuery.data.holidays.map(
      (holiday: IHoliday) =>
        holiday.date === formatDate(tileDate) &&
        (checkIsWorkingDay(holiday) ? (
          <Tooltip
            label="Working day"
  ...
  function holidayStatus(tileDate: Date) {
    if (filteredHolidays && checkboxState) {
      return filteredHolidays.map(
        (holiday: IHoliday) => holiday.date === formatDate(tileDate) && "public"
      );
    }
    const nonPublicHolidays = allHolidaysQuery.data.holidays.map(
      (holiday: IHoliday) =>
        holiday.date === formatDate(tileDate) && !holiday.public
          ? "non-public"
          : ""
    );
    const publicHolidays = allHolidaysQuery.data.holidays.map(
      (holiday: IHoliday) =>
        holiday.date === formatDate(tileDate) && holiday.public ? "public" : ""
    );
  ...
  useEffect(() => {
    if (allHolidaysQuery.data) {
      const filteredHolidays = allHolidaysQuery.data.holidays.filter(
        (holiday: IHoliday) => holiday.public === checkboxState
      );
  ...
  return (
    <div>
      ...
      {countryQuery.error &&
        <p>An error has occurred</p> + countryQuery.error.message}
      {countryQuery.data && (
        <CountryInfo
