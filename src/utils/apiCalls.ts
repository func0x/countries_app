...
export const getAllCountries = async () => {
  return axios
    .get(`${process.env.REACT_APP_ALL_COUNTRIES_URL}`)
...
export const getCountryByName = async (country: string) => {
  if (country !== "") {
    return axios
      .get(`${process.env.REACT_APP_COUTRY_BY_NAME_URL}${country}`)
      .then((response) => response.data)
      .catch((error) => console.error(error));
  }
};
...
export const getCountryByAlpha2Code = async (alpha2Code: string) => {
  return axios
    .get(`${process.env.REACT_APP_COUNTRY_BY_ALPHA2CODE_URL}${alpha2Code}`)
    .then((response) => response.data)
    .catch((error) => console.error(error));
};
...
export const getAllHolidays = async (alpha2Code: string, lang: string) => {
  return axios
    .get(
      `${
        process.env.REACT_APP_ALL_HOLIDAYS_URL
      }?key=${API_KEY}&country=${alpha2Code.toUpperCase()}&language=${lang}&year=2020`
    )
    .then((response) => response.data)
    .catch((error) => console.error(error));
};
...
export const getAvailableHolidaysLanguages = async () => {
  return axios
    .get(
      `${process.env.REACT_APP_AVAILABLE_HOLIDAYS_LANGUAGES_URL}?key=${API_KEY}`
    )
