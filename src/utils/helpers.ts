...
export function checkLanguageFallback(lang: string, data: ILanguage[]): string {
  const checkAvailability =
    data && data.filter((language: ILanguage) => language.code === lang);
  const langAfterCheckFallback =
    checkAvailability.length > 0 ? checkAvailability[0].code : "en";
...
export function checkIsWorkingDay(holiday: IHoliday): boolean {
  return (
    !holiday.public &&
    !holiday.public &&
    holiday.weekday.date.numeric !== "6" &&
    !holiday.public &&
    holiday.weekday.date.numeric !== "7"
  );
}
